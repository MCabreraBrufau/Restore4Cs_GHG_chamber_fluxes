...
#static chamber incubation from the RESTORE4Cs project. The best-model for each incubation is 
...
#strong artefacts resulting from ebullition are discarded and only LM and HM models are
#considered for the rest of incubations).
...
#3. IF NO bubbles are found (strictly diffusive dynamics), the following logic applies to decide 
#between LM and HM model, named as "goFlux criteria" throughout this script: 
...
#LM as best.model. This avoids assigning a relatively high-magnitude HM flux due to noise #producing a strongly curved HM fit that is not supported by incubation design.
...
#based on the distribution for the best-performing HM models, and systematically inspect #incubations that exceed this threshold to ensure that no "truly non-linear" dynamics is 
...
#This selection implements the same approach used for CH4 to CO2 incubations. It implements the same criteria used for CH4, re-defining the g.fact threshold to the CO2 dynamics. 
